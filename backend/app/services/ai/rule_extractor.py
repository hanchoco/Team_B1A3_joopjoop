...
# "최선의 추측" 구현입니다. process_policy()가 실제 로직이고, 아래는
# 그 결과를 다른 이름/형태로 감싼 것뿐입니다.
# ============================================================
...
class ExtractedCondition:
    """policy_conditions 한 행을 담는 타입. process_policy()가 만든 dict와 필드가 같습니다."""
    condition_key: str
    operator: str
...
def extract_conditions(raw_policy: dict) -> list:
    """process_policy(raw_policy)의 policy_conditions 부분만 ExtractedCondition 리스트로 반환."""
    result = process_policy(raw_policy)
    valid_fields = ExtractedCondition.__dataclass_fields__.keys()
    return [
        ExtractedCondition(**{k: v for k, v in c.items() if k in valid_fields})
        for c in result["policy_conditions"]
    ]
...
def validate_condition_payload(payload: dict) -> bool:
    """condition_key가 허용된 레지스트리(profile.*/employment.*/participation_limit) 안에 있는지만 확인."""
    return payload.get("condition_key") in (ALLOWED_CONDITION_KEYS | CHECKBOX_ONLY_CONDITION_KEYS)