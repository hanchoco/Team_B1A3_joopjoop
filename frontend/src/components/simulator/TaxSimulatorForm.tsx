...
export default function TaxSimulatorForm({ values, onChange }: SimulatorFormProps) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-semibold">
        최대 감면액(원)
        <input name="maxReduction" type="number" min={0} value={values.maxReduction ?? 0} onChange={update} className="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 font-normal" />
      </label>
      <label className="text-sm font-semibold">
        적용 기간(개월)
